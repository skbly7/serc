        ...
        for day in cal.itermonthdays(self.year, self.month):
            ...
            if day:
                ...
                occurrences = filter(
                    ...
                    all_occurrences)
                # occurrences = filter(
                # lambda occ, date=date: (date > occ.start.replace(hour=0, minute=0, second=0, microsecond=0) and date < occ.end.replace(hour=0, minute=0, second=0, microsecond=0) or (date == occ.start.replace(hour=0, minute=0, second=0, microsecond=0)) or (date == occ.end.replace(hour=0, minute=0, second=0, microsecond=0))),
                # all_occurrences)
